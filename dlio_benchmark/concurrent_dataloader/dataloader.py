...
from dlio_benchmark.concurrent_dataloader.fetch import _AsyncMapDatasetFetcher
from dlio_benchmark.concurrent_dataloader.fetch import _IterableDatasetFetcher
from dlio_benchmark.concurrent_dataloader.fetch import _MapDatasetFetcher
from dlio_benchmark.concurrent_dataloader.fetch import _ThreadedMapDatasetFetcher
from dlio_benchmark.concurrent_dataloader.worker import get_worker_info
from dlio_benchmark.concurrent_dataloader.worker import _worker_loop as _worker_loop_parallel
import dlio_benchmark.concurrent_dataloader.worker
...
class _DatasetKind(object):
    ...
    @staticmethod
    def create_fetcher(kind, dataset, auto_collation, collate_fn, drop_last, fetch_impl: str, num_fetch_workers: int = 1):
        logging.info(f"fetcher is {fetch_impl}")
...
class _SingleProcessDataLoaderIter(_BaseDataLoaderIter):
    def __init__(self, loader):
        ...
        self._dataset_fetcher = _DatasetKind.create_fetcher(
            self._dataset_kind, self._dataset, self._auto_collation, self._collate_fn, self._drop_last,loader.fetch_impl, loader.num_fetch_workers)
...
class _MultiProcessingDataLoaderIter(_BaseDataLoaderIter):
    ...
    def __init__(self, loader):
        ...
        for i in range(self._num_workers):
            ...
            logging.info(f"worker {i} fetcher is {loader.fetch_impl}")
            w = multiprocessing_context.Process(
                target=_worker_loop_parallel,
    ...
    def _reset(self, loader, first_iter=False):
        ...
        if not first_iter:
            for idx in range(self._num_workers):
                self._index_queues[idx].put(dlio_benchmark.concurrent_dataloader.worker._ResumeIteration(self._shared_seed))
            resume_iteration_cnt = self._num_workers
            while resume_iteration_cnt > 0:
                return_idx, return_data = self._get_data()
                if isinstance(return_idx, dlio_benchmark.concurrent_dataloader.worker._ResumeIteration):
...
# (shell2) ./test_socket.py sock_tmp 1017 send
    ...
    def _next_data(self):
        while True:
            ...
            if self._dataset_kind == _DatasetKind.Iterable:
                ...
                if isinstance(data, dlio_benchmark.concurrent_dataloader.worker._IterableDatasetStopIteration):
    ...
    def _try_put_index(self):
        ...
        # logging.info(f"send_idx: {self._send_idx}, index len : {len(index)}")
