...
from dlio_benchmark.concurrent_dataloader.dataloader import DataLoader as DataLoaderParallel
...
class MegatronDataset(Dataset):
    ...
    def __init__(self, format_type, dataset_type, epoch, num_samples, num_workers, batch_size):
        logging.info(f"{utcnow()} Initiating...")
...
class megatron_sampler(Sampler):
    def __init__(self, rank, size, num_samples, shuffle, epochs, seed, batch_size):
        self.size = size
        self.rank = rank
        self.num_samples = num_samples
        self.shuffle = shuffle
    ...
    def __iter__(self):
        ...
        samples_per_gpu = self.num_samples // self.size
        start = self.rank * samples_per_gpu
        end = min((self.rank + 1) * samples_per_gpu, self.num_samples)
...
class MegatronDataLoader(BaseDataLoader):
    ...
    @dlp.log
    def read(self):
        dataset = MegatronDataset(self.format_type, self.dataset_type, self.epoch_number, self.num_samples,
        ...
        if torch.__version__ == '1.3.1':
            ...
            #                            **kwargs)
            self._dataset = DataLoaderParallel(dataset,
                                       batch_size=self.batch_size,
                                       sampler=sampler,
                                       num_workers=self._args.read_threads,
                                       pin_memory=True,
                                       drop_last=True,
                                       worker_init_fn=dataset.worker_init, 
                                       num_fetch_workers=self._args.num_fetch_workers,
                                       fetch_impl=self._args.fetch_impl,
                                       batch_pool=self._args.batch_pool,
                                       **kwargs)
        else: 
            # self._dataset = DataLoader(dataset,
            #                            batch_size=self.batch_size,
