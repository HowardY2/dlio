...
class ConfigArguments:
    ...
    def derive_configurations(self, file_list_train=None, file_list_eval=None):
        ...
        if self.data_loader_sampler is None and self.data_loader_classname is None:
            ...
            elif self.data_loader in [DataLoaderType.PYTORCH, DataLoaderType.DALI, DataLoaderType.MEGATRON]:
    ...
    def initialize_mappings(self, file_list):
        current_id = 0
        self.id_to_path = {}
        self.path_to_id = {}
        for path in file_list:
            abs_path = os.path.abspath(path)  # 确保使用绝对路径进行编码
            if abs_path not in self.path_to_id:
                self.path_to_id[abs_path] = current_id
                self.id_to_path[current_id] = abs_path
                current_id += 1
    ...
    def get_global_map_index(self, file_list, total_samples):
        ...
        self.initialize_mappings(file_list)

        for global_sample_index in range(total_samples):
            # logging.info(f"{utcnow()} rank: {self.my_rank} global smaple index is {global_sample_index}")
            file_index = global_sample_index//self.num_samples_per_file
            file_id = self.encode_path(file_list[file_index])
            # abs_path = os.path.abspath(file_list[file_index]) 
            ...
            process_thread_file_map[global_sample_index] = (file_id, sample_index)
...
def LoadConfig(args, config):
    ...
    if reader is not None:
        ...
        if 'num_fetch_workers' in reader:
            args.num_fetch_workers = reader['num_fetch_workers']
        if 'fetch_impl' in reader:
            args.fetch_impl = reader['fetch_impl']
        if 'batch_pool' in reader:
            args.batch_pool = reader['batch_pool']
